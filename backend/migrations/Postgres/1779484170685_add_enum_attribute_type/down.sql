------------------------------------------------------------------------------------------------------------------------
-- Revert ENUM attribute data type
------------------------------------------------------------------------------------------------------------------------

-- 1. drop the option-rename cache propagation
drop trigger if exists cached_attributions_enum_options_trigger on attribute_enum_options;
drop function if exists update_cached_attributions_from_enum_options();

-- 2. restore the cache functions to their pre-ENUM definitions
...
begin
    ...
    if tg_op = 'INSERT' then
        insert into cached_attributions (
            id, integer_value, float_value, text_value, boolean_value, date_value, text_note, photo_note,
            exceptional_attribution, attribute_name, attribute_id, data_type, attribute_type, attribution_id,
            plant_id, plant_group_id, cultivar_id, lot_id, attribution_form_id, combined_plant_group_id,
            combined_cultivar_id, created, modified, author, date_attributed, geo_location, geo_location_accuracy
        ) values (
            new.id, new.integer_value, new.float_value, new.text_value, new.boolean_value, new.date_value,
            new.text_note, new.photo_note, new.exceptional_attribution, attribute_rec.name, new.attribute_id,
            attribute_rec.data_type, attribute_rec.attribute_type, new.attribution_id, attribution_rec.plant_id,
            attribution_rec.plant_group_id, attribution_rec.cultivar_id, attribution_rec.lot_id,
            attribution_rec.attribution_form_id, combined_ids.combined_plant_group_id,
            combined_ids.combined_cultivar_id, new.created, new.modified, attribution_rec.author,
            attribution_rec.date_attributed, attribution_rec.geo_location, attribution_rec.geo_location_accuracy
        );
...
-- 3. restore the attribution value validation to its pre-ENUM definition
...
-- 4. restore the attribute validation functions
...
-- 5. drop the value column (also drops the composite FK and its index) and the options table
alter table attribution_values drop column if exists attribute_enum_option_id;

drop table if exists attribute_enum_options;
drop function if exists check_enum_option_attribute();

-- 6. remove the data type (fails if any ENUM attribute still exists)
delete from attribute_data_types where enum = 'ENUM';

-- 7. rebuild the cache with the restored logic
