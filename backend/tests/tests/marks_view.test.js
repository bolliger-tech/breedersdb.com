...
describe("non aggregated values are correct", async () => {
  test("common columns", async () => {
    ...
    expect(data.marks_view[0].name).toBe("Attribute 1");
...
describe("aggregated values are correct", async () => {
  test("INTEGER", async () => {
    const { attribute_id, mark_id } =
      await insert_mark_value_with_associated_data({
        is_lot: true,
        integer_value: 1,
        mark_attribute_data_type: "INTEGER",
    ...
    const { data } = await post({
      query: queryByAttributeId,
      variables: {
        mark_attribute_id: attribute_id,
      },
    });

    expect(data.marks_view).toHaveLength(3);
    expect(data.marks_view[0].count).toBe(3);
    expect(data.marks_view[0].min).toBe(1);
    expect(data.marks_view[0].max).toBe(6);
    expect(data.marks_view[0].avg).toBe(3);
    expect(data.marks_view[0].stddev).toBeCloseTo(2.16, 2);
    expect(data.marks_view[0].median).toBe(2);
  });
  ...
  test("FLOAT", async () => {
    ...
    expect(data.marks_view).toHaveLength(3);
    expect(data.marks_view[0].count).toBe(3);
    expect(data.marks_view[0].min).toBe(0.1);
    expect(data.marks_view[0].max).toBe(0.6);
    expect(data.marks_view[0].avg).toBe(0.3);
    expect(data.marks_view[0].stddev).toBeCloseTo(0.216, 3);
    expect(data.marks_view[0].median).toBe(0.2);
  });
  ...
  test("BOOLEAN", async () => {
    ...
    expect(data.marks_view).toHaveLength(3);
    expect(data.marks_view[0].count).toBe(3);
    expect(data.marks_view[0].any_true).toBe(true);
    expect(data.marks_view[0].all_true).toBe(false);
  });
  ...
  test("DATE", async () => {
    ...
    expect(data.marks_view).toHaveLength(3);
    expect(data.marks_view[0].count).toBe(3);
    expect(data.marks_view[0].date_min).toBe("2021-01-01");
    expect(data.marks_view[0].date_max).toBe("2021-01-06");
  });

  test("doesn't contain other attributes", async () => {
    const { attribute_id, mark_id } =
      await insert_mark_value_with_associated_data({
        is_lot: true,
        integer_value: 1,
    ...
    const attribute = await post({
      ...
      variables: {
        ...
        validation_rule: { min: 0, max: 100, step: 1 },
        data_type: "INTEGER",
    ...
    expect(data.marks_view).toHaveLength(2);
    expect(data.marks_view[0].count).toBe(2);
    expect(data.marks_view[0].min).toBe(1);
    expect(data.marks_view[0].max).toBe(2);
    expect(data.marks_view[0].avg).toBe(1.5);
    expect(data.marks_view[0].stddev).toBeCloseTo(0.5, 2);
    expect(data.marks_view[0].median).toBe(1.5);
  });

  test("lots only contain lot marks", async () => {
    const { attribute_id, mark_id, form_id, cultivar_id, lot_id, tree_id } =
      await insert_mark_value_with_associated_data({
        is_lot: true,
        integer_value: 1,
        mark_attribute_data_type: "INTEGER",
    ...
    // must NOT be aggregated
    const cultivarMark = await post({
      query: insertMark,
      variables: {
        author: "Author 2",
        date_marked: "2021-01-02",
        mark_form_id: form_id,
        cultivar_id: cultivar_id,
      },
    });
    await post({
      ...
      variables: {
        mark_attribute_id: attribute_id,
        mark_id: cultivarMark.data.insert_marks_one.id,
        integer_value: 1,
      },
    });
    const treeMark = await post({
      query: insertMark,
      variables: {
        author: "Author 2",
        date_marked: "2021-01-02",
    ...
    await post({
      ...
      variables: {
        mark_attribute_id: attribute_id,
        mark_id: treeMark.data.insert_marks_one.id,
        integer_value: 6,
      },
    });
    ...
    const { data } = await post({
      query: queryByAttributeIdAndLotId,
      variables: {
        mark_attribute_id: attribute_id,
        lot_id: lot_id,
      },
    ...
    expect(data.marks_view[0].stddev).toBeCloseTo(0.5, 2);
    expect(data.marks_view[0].median).toBe(1.5);
    expect(data.marks_view[0].stats_source).toBe('LOTS');
  });

  test("cultivars contain tree marks but not lot marks", async () => {
    const { attribute_id, mark_id, form_id, cultivar_id, tree_id, lot_id } =
      await insert_mark_value_with_associated_data({
        is_cultivar: true,
        integer_value: 1,
        mark_attribute_data_type: "INTEGER",
    ...
    // must NOT be aggegated
    const lotMark = await post({
      query: insertMark,
      variables: {
        author: "Author 2",
        date_marked: "2021-01-02",
    ...
    await post({
      ...
      variables: {
        mark_attribute_id: attribute_id,
        mark_id: lotMark.data.insert_marks_one.id,
        integer_value: 3,
      },
    });
    ...
    const aggData = data.marks_view.find(row => row.stats_source === 'TREES_AND_CULTIVARS')

    expect(data.marks_view).toHaveLength(3);
    expect(aggData.count).toBe(3);
    expect(aggData.min).toBe(1);
    expect(aggData.max).toBe(6);
    expect(aggData.avg).toBe(3);
    expect(aggData.stddev).toBeCloseTo(2.16, 2);
    expect(aggData.median).toBe(2);
    expect(aggData.stats_source).toBe('TREES_AND_CULTIVARS');
  });

  test("trees only contain tree marks", async () => {
    const { attribute_id, mark_id, form_id, cultivar_id, lot_id, tree_id } =
      await insert_mark_value_with_associated_data({
        is_tree: true,
        integer_value: 1,
    ...
    expect(data.marks_view[0].stddev).toBeCloseTo(0.5, 2);
    expect(data.marks_view[0].median).toBe(1.5);
    expect(data.marks_view[0].stats_source).toBe('TREES');
  });
});
